...
/*revised soojin
				...
				capture = cvCaptureFromFile("car1.mp4");
...
*/
                ...
                /*
				cv::Mat m = cv::imread("car.jpg", cv::IMREAD_COLOR);

				std::cout <<  "Hello from save_box" << std::endl;
                
				cv::Rect rect(left,top,318,83);
				cv::Mat subImg = m(rect);
				...
				sprintf(saved, "car_%d.jpg", cnt);
				imwrite(saved, subImg);
                ...
                if (!save_flag) {
                    ...
                    char saved[100] = { 0 };
                    memset(saved, 0, 100);
                    sprintf(saved, "car_%d.jpg", i);
                    imwrite(saved, subImg);
                }
                */
                std::ifstream readFile;
                readFile.open("car.txt");

                int len_cnt = 0;
                std::string t;
                while (getline(readFile, t, '\n'))
                    len_cnt++;
                std::cout << len_cnt << std::endl;
                if (len_cnt < 3) {
                    cv::Mat m = cv::imread("car.jpg", cv::IMREAD_COLOR);

                    std::ofstream of;
                    of.open("car.txt", std::ios::app);
                    char position[100] = { 0 };
                    memset(position, 0, 100);
                    if (len_cnt == 0) {
                        of << m.rows << " " << m.cols << endl;
                    }
                  
                    sprintf(position, "A %d %d", left, top);
                    of << position << endl;
                    ...
                    char saved[100] = { 0 };
                    memset(saved, 0, 100);
                    sprintf(saved, "car_%d.jpg", len_cnt);
                    imwrite(saved, subImg);
                }
            }
        }
